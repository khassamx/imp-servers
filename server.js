...
function loadMessages() {
    try {
        ...
        // Si el archivo está vacío o tiene caracteres no-JSON, puede fallar aquí.
        if (data.trim() === '') return []; 
        return JSON.parse(data);
    } catch (error) {
        // Ignorar error ENOENT (archivo no existe) o error de parseo, y retornar vacío
        if (error.code !== 'ENOENT') {
            console.error("Error al cargar messages.json:", error);
        }
        return [];
    }
}
...
app.post('/messages', (req, res) => {
    ...
    // Añadir el mensaje y la marca de tiempo 
    messages.push({ ...newMessage, timestamp: new Date().toISOString() });
    
    saveMessages(messages);
    
    // Devolvemos el array completo al frontend.
